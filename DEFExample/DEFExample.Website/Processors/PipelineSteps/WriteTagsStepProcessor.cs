...
namespace DEFExample.Website.Processors.PipelineSteps
{
    ...
    public class WriteTagsStepProcessor : BasePipelineStepProcessor
    {
            ...
            ILogger logger)
        {
            ...
            try
            {
                ...
                var tags = dataSet?.Data?.Cast<Models.Tag>().ToList();
                _wordpressService.CreateTagsInSitecore(tags, logger);
            }
            catch (Exception ex)
            {
                logger.Error($"Error in WriteTagsStepProcessor: {ex.InnerException}");
